...
def callback(input_data):
	...
	if current_time - last_call_time > wait_time:
		...
		# sprayer logic
		temp = 0
		if int(channel(7)) >= 1050:
			temp += 1
		if int(channel(8)) >= 1050:
			temp += 2
		if int(channel(9)) >= 1050:
			temp += 4
		if int(channel(10)) >= 1050:
			temp += 8
		sendSprayer(temp)
		if int(channel(5)) <= 1050:  # 1050 is an arbitrary number for a threshold
			manual_control()
		else:
			return
		return
...
def sendSprayer(data):
	Sprayer = rospy.Publisher('/Sprayer', Int8, queue_size=1)
	Sprayer.publish(data)
	return


# Bless https://www.instructables.com/Joystick-to-Differential-Drive-Python/
def joystickToDiff(x, y, minJoystick, maxJoystick, minSpeed,
                   maxSpeed):  # If x and y are 0, then there is not much to calculate...
    if x == 0 and y == 0:
        return (0, 0)
